...
Есть ли такое значение , менше 0

        any(avian$PDB < 0)
Пишем функцию для проверки у других
        
        check_pecent_range <-function(x){
            any(x < 0 || x > 100)
        }
        
 Смотрим имена перменных        
    
        names(avian)
 Берем только те, которые содержат % 
 
        coverage_variables <- names(avian)[-(1:4)][c(T,F)]
  Записываем Сумму в новую переменную
